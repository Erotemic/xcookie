...
class GitLabCIRenderer:
    ...
    def render_checks(self) -> str:
        """Render standalone project-owned source checks."""
        return make_gitlab_source_checks(self.applier, plan=self.plan)

    def render_manifest(self) -> str:
        """Render the root include manifest used when source checks are split."""
        ...
        header = ub.codeblock(
            """
            # Autogenerated by xcookie.
            # The anchor-heavy main pipeline remains self-contained because
            # GitLab YAML anchors cannot cross included files.
            """
        )
...
def make_gitlab_source_checks(self, plan: CIPlan | None = None) -> str:
    """Render source checks without depending on anchors from the main file."""
    if plan is None:
    ...
    tags = ['docker', 'linux-x86_64', 'build']
    if 'kitware' in self.tags:
        tags.append('kitware-python-stack')
    ...
    for check in plan.ci_source_checks:
        ...
        if image is None:
            image = f'python:{python_version or main_python_version}'
        ...
        job = {
            ...
            'variables': variables,
            'except': {'refs': ['release']},
            'cache': {'paths': ['.cache/pip', '.cache/uv']},
            'script': [*check.setup_commands, *check.commands],
        }
        if check.allow_failure:
            job['allow_failure'] = True
        body[f'check/{check.key}'] = CommentedMap(job)

    header = ub.codeblock(
        """
        # Autogenerated by xcookie.
        # Project-owned source checks are intentionally isolated from the
        # anchor-heavy primary GitLab pipeline.
        """
    )
    return (header + '\n' + Yaml.dumps(body)).rstrip() + '\n'
...
def build_gitlab_ci_main(self):
    return GitLabCIRenderer(self).render_main()


def build_gitlab_ci_checks(self):
    return GitLabCIRenderer(self).render_checks()
